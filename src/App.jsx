    ...
    const cleanups = elements.map((element, index) => {
      ...
      return inView(element, () => {
        animate(
          ...
          { duration: 0.65, delay: Math.min((index % 4) * 0.055, 0.16), ease: [0.22, 1, 0.36, 1] },
        )
...
export default function App() {
  ...
  return <MotionConfig reducedMotion="user" transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}><div className="site-shell">
    <ScrollToTop />
    ...
    <header className="header">
      ...
      <AnimatePresence initial={false}>
        {open && <motion.nav className="mobile-nav open" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }}>
          {navItems.map(([label, to], index) => <motion.div key={to} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.035 }}><NavLink to={to} onClick={() => setOpen(false)}>{label}</NavLink></motion.div>)}
          <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.18 }}><NavLink className="button button-lime" to="/contact" onClick={() => setOpen(false)}>Book a test</NavLink></motion.div>
        </motion.nav>}
      </AnimatePresence>
    </header>
    <main><AnimatePresence mode="wait" initial={false}><motion.div key={location.pathname} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}><ScrollReveals><Routes location={location}>
    ...
    </Routes></ScrollReveals></motion.div></AnimatePresence></main>
